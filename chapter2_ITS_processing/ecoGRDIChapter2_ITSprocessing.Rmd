---
title: 'Chaper 2: ITS Sample Processing Raw Reads'
author: "Emily Giroux"
date: "4/2/2019"
output:
  html_document:
...
We're going to use Cutadapt to detect and trim off ITS-specific adapter sequences. Cutadapt uses Biostrings, which only recognises the main IUPAC codes. We need to replace any extended IUPAC nucleic acid ambiguity codes to the primary codes. In our ITS primers we see the following ambiguiity codes:           
R is for A or G     
Y is for C or T     
...
>\textcolor{blue}{5'} GAACGCAGCRAA\textcolor{red}{\textbf{II}}GYGA\textcolor{blue}{3'}          
     
We'll replace the \textcolor{red}{\textbf{I}}s with \textcolor{green}{\textbf{B}}s to get:          
     
>\textcolor{blue}{5'} GAACGCAGCRAA\textcolor{green}{\textbf{BB}}GYGA \textcolor{blue}{3'}           
    
No changes are required for the reverse primer as there are no extended ambiguity codes in the primer sequence:    
**ITS4R:**      
     
>\textcolor{blue}{5'} TCCTCCGCTTATTGATATGC \textcolor{blue}{3'}           
...
```
Before checking our ITS sequences for primers with cutadapt, we need to remove those sequences with ambiguous bases. Ambiguous bases (Ns) in the sequencing reads makes accurate mapping of short primer sequences difficult, so we "pre-filter" these sequences so that we only remove those with Ns and perform no other fitlering.
...
As a precaution, we still check for samples with no remaining reads and update the metadata table for our ITS samples using those results. If you receive an error during the dereplication step later on: `Error in open.connection(con, "rb") : cannot open the connection`,
