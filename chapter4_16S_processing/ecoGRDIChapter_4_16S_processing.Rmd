...
if(any(!.inst)) {BiocManager::install(.bioc_packages[!.inst], ask = FALSE)}
```
...
**Load the saved image from Chapter 1**, and create and save the environment data specific to the 16S processing and analysis workflow.
```{r, loadBaseImage, echo=TRUE, eval=FALSE, include=TRUE, results='hold'}
load(paste(imageDirPath, thisImage, sep = ""))
chptImage <- "ecobiomics_16S.RData"
save.image(paste(imageDirPath, chptImageA, sep = ""))
```
...
```
Use the AllOrients function to generate the primer sequences in all possible orientations in which they may be found:
...
We can now check the N-filtered sequences of just one sample set for primer hits using the PrimerHits function:
...
foreach::foreach(i = 1:length(metadataRegion$LibraryName), .errorhandling = 'pass') %dopar% {
