---
title: "GRDI Ecobiomics 18S Analysis"
author: "Emily Giroux"
date: "11/19/2021"
output: pdf_document
fontsize: 11pt
...
```{r}
chptImage    <- "ecobiomics_18S_analysis.RData"
...
library("ggplot2")
library("cowplot")
all18SAlphaDIversity <- plot_richness(ps2Species,
                                      x = "ExtractionKit",
                                      measures = c("Shannon", "Simpson"),
                                      color = "Sample",
                                      title = "Alpha diversity of all 18S samples by extraction kit") +
  theme(axis.text.x = element_text(angle = 90)) +
  theme(plot.title = element_text(hjust = 0.5)) + 
  scale_x_discrete(name = "Extraction Kit")
png(file = paste(ggplotsPath, "/All_18S_AlphaDiversityShannonSimpson", ".png", sep = ""))
print(all18SAlphaDIversity)
dev.off()
all18SAlphaDIversity
```
...
png(file = paste(ggplotsPath, "/PCoA_distanceBray_18S_all", ".png", sep = ""))
pcoa <- phyloseq::plot_ordination(logt, out.pcoa.logt, type = "Sample", 
                                  color = "ExtractionKit") + 
  ggplot2::labs(col = "ExtractionKit") +
  coord_fixed(sqrt(evals[2] / evals[1])) +
  ggtitle("PCoA distance Bray ordinate plot for genera -vs- extraction kit for all 18S samples") +
  theme(plot.title = element_text(hjust = 0.5))
print(pcoa)
dev.off()
pcoa
...
library("ggplot2")
png(file = paste(ggplotsPath, "/alphaDiversity_corrected_18S_all", ".png", sep = ""))
...
```{r}
png(file = paste(ggplotsPath, "/brayNMDS_ordinationPlot_18S_all", ".png", sep = ""))

brayNMDS <- plot_ordination(ps.prop, ord.nmds.bray, color="ExtractionKit", title="Bray NMDS") +
  ggtitle("Bray NMDS of all 18S samples by extraction kit") +
  theme(plot.title = element_text(hjust = 0.5))
...
# Therefore, in the final phyloseq, the main phyla are Opisthokonta, SAR, Amoebozoa and Archaeplastida.    
```
```{r}
df2 <- psmelt(ps.top20)
...
```{r}
png(file = paste(ggplotsPath, "/top20SpeciesAbundanceSciNames_18S_all", ".png", sep = ""))
sppAbundanceTop20_SciNames <- ggplot(df2, aes_string(x = "ExtractionKit", y = "Abundance", fill = "Species"),
                                     title = "Abundance of top 20 species in\n18S samples by extraction kit",
                                     facet_grid = ~sample_Sample) +
  geom_bar(aes(color = Species, fill = Species), stat = "identity", position = "stack") +
  ggtitle("Abundance of top 20 species in 18S samples by extraction kit") +
...
opisthokonta <- c("Neoptera", "Chromadorea", "Eutardigrada")
...
```{r}
library("ggplot2")
png(file = paste(ggplotsPath, "/relativeAbundance_all_18S", ".png", sep = ""))
...
png(file = paste(ggplotsPath, "/AbundanceMainPhylumsFamilies_all_18S", ".png", sep = ""))
