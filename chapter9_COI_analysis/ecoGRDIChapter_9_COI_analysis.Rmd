---
title: "GRDI Ecobiomics COI Analysis"
author: "Emily Giroux"
date: "11/25/2021"
output: pdf_document
fontsize: 11pt
...
Recall from Chapter 8 COI DADA2-specific processing step: The only variables we need from the previous environment for this analysis chapter are the "ps" variable from the fitGTR image, which we re-saved in the chptImageB variable:
```{r}
chptImageB <- "ecobiomics_COI_2b.RData"
load(paste(imageDirPath, chptImageB, sep = ""))
```

Fresh image for this new chapter, and image to continue saving going forward:
```{r}
chptImage    <- "ecobiomics_COI_analysis.RData"
save.image(paste(imageDirPath, chptImage, sep = ""))
load(paste(imageDirPath, chptImage, sep = ""))
```

For COI we can analyse the data at the species level.      
...
library(ggplot2)
library(cowplot)
allCOIAlphaDIversity <- plot_richness(ps2Species,
                                      x = "ExtractionKit",
                                      measures = c("Shannon", "Simpson"),
                                      color = "Sample",
                                      title = "Alpha diversity of all COI samples by extraction kit") +
  theme(axis.text.x = element_text(angle = 90)) +
  theme(plot.title = element_text(hjust = 0.5)) + 
  scale_x_discrete(name = "Extraction Kit")
png(file = paste(ggplotsPath, "/All_COI_AlphaDiversityShannonSimpson", ".png", sep = ""))
print(allCOIAlphaDIversity)
dev.off()
allCOIAlphaDIversity
```
...
png(file = paste(ggplotsPath, "/PCoA_distanceBray_COI_all", ".png", sep = ""))
pcoa <- phyloseq::plot_ordination(logt, out.pcoa.logt, type = "Sample", 
                                  color = "ExtractionKit") + 
  ggplot2::labs(col = "ExtractionKit") +
  coord_fixed(sqrt(evals[2] / evals[1])) +
  ggtitle("PCoA distance Bray ordinate plot for genera -vs- extraction kit for all COI samples") +
  theme(plot.title = element_text(hjust = 0.5))
print(pcoa)
dev.off()
pcoa
...
library("ggplot2")
png(file = paste(ggplotsPath, "/alphaDiversity_corrected_COI_all", ".png", sep = ""))
...
```{r}
png(file = paste(ggplotsPath, "/brayNMDS_ordinationPlot_COI_all", ".png", sep = ""))

brayNMDS <- plot_ordination(ps.prop, ord.nmds.bray, color="ExtractionKit", title="Bray NMDS") +
  ggtitle("Bray NMDS of all COI samples by extraction kit") +
  theme(plot.title = element_text(hjust = 0.5))
...
```{r}
# Bar plot:
top20 <- names(sort(taxa_sums(logt), decreasing=TRUE))[1:102] # Some unique asvs may group together at the species level and thus decrease the top 20 count, so manually increase this until 20 species get included in the abundance graph.
...
# Therefore, in the final phyloseq, the only phyla are Eukaryota and Bacteria.    
```
```{r}
df2 <- psmelt(ps.top20)
df2 %>% dplyr::count(Phylum) %>% arrange(desc(n))
# Therefore, in the top 20 species, the phyla fall under Eukaryota and Bacteria.
...
```{r}
png(file = paste(ggplotsPath, "/top20SpeciesAbundanceSciNames_COI_all", ".png", sep = ""))
sppAbundanceTop20_SciNames <- ggplot(df2, aes_string(x = "ExtractionKit", y = "Abundance", fill = "ScientificName"),
                                     title = "Abundance of top 16 species for all COI samples by extraction kit",
                                     facet_grid = ~sample_Sample) +
  geom_bar(aes(color = ScientificName, fill = ScientificName), stat = "identity", position = "stack") +
  ggtitle("Abundance of top 16 species for all COI samples by extraction kit") + 
  #theme(plot.title = element_text(hjust = 0.5)) +
...
# Therefore, in the final phyloseq, the most abundant phyla fall under Eukaryota (majority) and Bacteria.    
```{r}
phylums <- c("Eukaryota", "Bacteria")
df3 <- df
df3 %>% dplyr::count(Phylum, Family) %>% arrange(desc(n))
df3 %>% dplyr::count(Phylum, Family) %>% arrange(desc(n)) %>% filter(Phylum == "Eukaryota")
df3 %>% dplyr::count(Phylum, Family) %>% arrange(desc(n)) %>% filter(Phylum == "Bacteria")
```

```{r}
eukaryota <- c("Insecta", "Oomycetes", "Bacillariophyceae")
bacteria <- c("Alphaproteobacteria", "Betaproteobacteria", "Gammaproteobacteria")
```
...
df3$Family[df3$Phylum=="Eukaryota" & 
 !df3$Family %in% eukaryota] <- "Other Eukaryota"

df3$Family[df3$Phylum=="Bacteria" &
 !df3$Family %in% bacteria] <- "Other Bacteria"
```
...
```{r}
library(ggplot2)
png(file = paste(ggplotsPath, "/relativeAbundance_all_COI", ".png", sep = ""))
...
png(file = paste(ggplotsPath, "/AbundanceMainPhylumsFamilies_all_COI", ".png", sep = ""))
